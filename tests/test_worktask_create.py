...
WORKTASK_CREATE_URL = reverse("club:worktask-create")


class TestPublicWorkTaskCreateView(TestCase):
    def test_worktask_create_login_required(self):
        response = self.client.get(WORKTASK_CREATE_URL)
        self.assertNotEqual(response.status_code, 200)


class TestPrivateWorkTaskCreateView(TestCase):
    def setUp(self):
        User = get_user_model()
        self.client = Client()
